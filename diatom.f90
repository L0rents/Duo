module diatom_module
  ...
  integer(ik),parameter   :: Nobjects = 17  ! number of different terms of the Hamiltonian
  ...
  character(len=wl),parameter :: CLASSNAMES(1:Nobjects)  = (/ character(len=wl):: "POTENTIAL","SPINORBIT","L2", "L+", &
                                                             "SPIN-SPIN", "SPIN-SPIN-O","BOBROT","SPIN-ROT", &
                                                             "(A)DIABATIC","LAMBDAOPQ", "LAMBDAP2Q","LAMBDAQ", "BOBVIB",  &
       ...
       case("SPIN-ORBIT","SPIN-ORBIT-X", &
          ...
          case("QUADRUPOLE")
             ...
             if (iquad>ncouples) then
                 write(out,"(2a,i4,a,i6)") trim(w),": Number of quadrupoles = ",iquad," exceeds the maximal allowed value", &
                                                      ncouples
                 call report ("Too many couplings given in the input for"//trim(w),.true.)
             endif
             !
       ...
       do igrid =1, ngrid
         ...
         method_choice: select case(solution_method)
           case ("5POINTDIFFERENCES")
             ...
             !
             ! The nondiagonal matrix elements are:
         ...
         do jlevel = 1,ilevel
           ...
           if (iverbose>=6) then
              if (icontrvib(ilevel)%istate/=icontrvib(jlevel)%istate.and.ilevel/=jlevel.and.abs(psipsi_t)>sqrt(small_)) then
                 write(out,"('orthogonality is broken : <',i4,'|',i4,'> (',f16.6,')')") ilevel,jlevel,psipsi_t
                 stop 'Brocken orthogonality'
              endif
              !
              if (ilevel==jlevel.and.abs(psipsi_t-1.0_rk)>sqrt(small_)) then
                 write(out,"('normalization is broken:  <',i4,'|',i4,'> (',f16.6,')')") ilevel,jlevel,psipsi_t
